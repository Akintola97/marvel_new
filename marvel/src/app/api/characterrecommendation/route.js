    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a helpful assistant that provides 10 Marvel character recommendations based on description.",
      },
      {
        role: "user",
      ...
      recommendations.map(async (rec) => {
        ...
        try {
          const marvelResponse = await axios.get(
            `https://gateway.marvel.com:443/v1/public/characters?name=${query}&ts=${ts}&apikey=${PUBLIC_KEY}&hash=${hash}`
          );
          if (marvelResponse.data.data.results.length > 0) {
            ...
            return {
              ...rec,
              id: itemDetails.id, // Ensure each recommendation includes a unique ID
              description:
                itemDetails.description || "No description available",
              comics: itemDetails.comics.items || [],
            ...
            return {
              ...rec,
              id: crypto.randomUUID(), // Generate a unique ID if no results found
          ...
          return {
            ...rec,
            id: crypto.randomUUID(), // Generate a unique ID if there's an error
            description: "No description available",
            comics: [],
            thumbnail: null,
          };
